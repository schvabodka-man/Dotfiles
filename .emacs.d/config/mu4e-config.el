...
(add-hook 'mu4e-headers-mode-hook
		  (lambda ()
			...
			(local-set-key (kbd "C-f") #'mu4e-headers-search)
			(local-set-key (kbd "C-M-f") #'mu4e-headers-search-edit)
			(local-set-key (kbd "C-S-f") 'mu4e-headers-search-narrow)
			(local-set-key (kbd "M-f") #'swiper)
			(define-key map (kbd ",") 'mu4e-headers-prev)
			(define-key map (kbd ".") 'mu4e-headers-next)
			(define-key map (kbd "<") 'mu4e-headers-prev-unread)
			(define-key map (kbd ">") 'mu4e-headers-next-unread)
			(define-key map (kbd "<C-up>") 'mu4e-headers-prev-unread)
			(define-key map (kbd "<C-down>") 'mu4e-headers-next-unread)
			(define-key map (kbd "C-,") 'mu4e-headers-prev-unread)
			(define-key map (kbd "C-.") 'mu4e-headers-next-unread)))
